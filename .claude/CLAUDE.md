...
ユーザーとの対話の中で、現在の作業状態に応じて次に実行すべきコマンドを**必ずサジェスト**すること。サジェストは返答の末尾に `> 💡 次のステップ: ...` の形式で付与する。

### 状態判定とサジェスト対応表

| 状態 | 判定方法 | サジェスト |
|------|---------|-----------|
| 持ちタスクなし | 会話にタスク着手の履歴がない / タスク完了直後 | `> 💡 次のステップ: /pick でタスクを選択` |
| タスク一覧を確認したい | ユーザーが「タスク」「何がある」等に言及 | `> 💡 /tasks でタスク一覧を確認できます` |
| タスク選択直後（実装前） | `/pick` でタスクを選んだ直後 | `> 💡 次のステップ: /review-req で要件レビュー → plan モードで計画立案` |
| 実装完了 | 全完了条件を満たした / ユーザーが「完了」「できた」等に言及 | `> 💡 次のステップ: /review-code でコードレビュー → /close でコミット＆完了処理` |
| コードレビュー完了（PASS/CONDITIONAL承認済み） | `/review-code` が PASS or ユーザー承認済み | `> 💡 次のステップ: /close でコミット＆完了処理` |
| `/close` 完了 | コミット・マージ/PR完了 | `> 💡 次のステップ: /pick で次のタスクを選択` |
| コードを変更したが品質に不安 | ユーザーが「レビュー」「確認」等に言及 | `> 💡 /review-code でQA・セキュリティ・一貫性の3視点レビューを実行できます` |
| 設定ファイルを変更した | `.claude/` や `.codex/` 配下を編集 | `> 💡 /sync-config で設定をリポジトリに同期できます` |
| master が古い可能性 | ユーザーが「最新化」「pull」等に言及 | `> 💡 /update-master で master を安全に最新化できます` |
| worktree が散らかっている | ユーザーが「worktree」「整理」等に言及 | `> 💡 /worktree-clear で不要な worktree を削除できます` |

### ルール

- **毎回の返答末尾**に、現在の状態に最も適したサジェストを1つ付与する
- 明らかに不要な場合（雑談、質問への回答のみ等）はサジェスト不要
- 複数の選択肢がある場合は最も優先度の高い1つに絞る（上の表の上にあるほど優先）
- コマンド名は必ずコードスパン（`` ` ` ``）で囲む
