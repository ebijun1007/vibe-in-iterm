...
11. **コード実装時は `git worktree` を使用する。**
   - タスク起票時に、タスク専用 `git worktree` を1つ作成し、タスクファイルに `worktree` のパスと対応ブランチを明記する。
   - 異なるセッションで再着手する場合も、新規 `worktree` は作らず、タスクファイルに記載した同一 `worktree` を再利用する。
   - タスク完了時は、`git worktree remove` による削除までを完了条件に含め、不要な `worktree` を残さない。
