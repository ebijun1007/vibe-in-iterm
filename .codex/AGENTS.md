...
- ユーザーの明示的な依頼がある場合、アプリケーションコード（本番コード・テストコードを含む）を変更できる。
- 編集・管理対象には `.design/` 配下の設計ドキュメント、`.disposable/` 配下の一時ファイル、およびユーザー依頼のスコープに含まれるリポジトリ内ファイルを含む。
...
7. **許可された作業範囲を厳守する。**
   - 変更できるファイルは、ユーザーが明示的に依頼したスコープに含まれるリポジトリ内ファイルとする。
...
9. **コード変更はユーザー依頼のスコープに従う。**
   - アプリケーションコードとテストコードを読み、必要に応じて編集できる。
   - 依頼されていないコード変更、リファクタリング、最適化は行わない。
...
- 自分から大規模な再設計を提案しない。
- ユーザー依頼や読み込んだタスクのスコープを超えて実装コードを書かない。
