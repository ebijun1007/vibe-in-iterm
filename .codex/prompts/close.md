...
1. `.design/tasks/` フォルダ内の `YYYYMMDD-*.md` ファイル（`README.md` を除く）を走査
2. `## ステータス` セクション内で `[x] 進行中` を含むファイルを検索
3. 0件なら「進行中のタスクが見つかりません」と報告して終了
4. 1件ならそのタスクを対象に、複数件ならユーザーに選択を促す

### 2. 変更内容の把握

1. `git status` と `git diff`（staged + unstaged）で変更ファイルを確認
2. `git log` で直近のコミット履歴を確認
3. タスクの完了条件と照らし合わせ、関連する変更を特定
